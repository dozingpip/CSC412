...
void initializeApplication(int argc, char** argv)
{
    ...
    for(int i = 0; i<numRows; i++)
    {
        mutex[i] = (pthread_mutex_t*) malloc(numCols*sizeof(pthread_mutex_t));
    }
...
void* threadFunc(void* param)
{
    ...
    int j = rand() % (numRows-1);
    int k = rand() % (numCols-1);
    ...
    for(int a = j-1; a<=j+1; a++){
        if(a>=0 && a<=numRows){
            ...
            {
                if(b>=0 && b<=numCols){
                    // printf("acquiring lock for %d %d\n", a, b);
                    pthread_mutex_lock(&mutex[a][b]);
                }
            }
        }
    }
    ...
    for(int a = j-1; a<=j+1; a++){
        if(a>=0 && a<=numRows){
            ...
            {
                if(b>=0 && b<=numCols){
                    // printf("releasing lock for %d %d\n", a, b);
                    pthread_mutex_unlock(&mutex[a][b]);
                }
            }
        }
    }
