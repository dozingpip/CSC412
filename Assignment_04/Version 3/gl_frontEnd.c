...
extern int timeBetweenGenerations;

const int SPEED_INCREMENT = 10000;

FILE *pipeFilePointer;
...
void myKeyboard(unsigned char c, int x, int y)
{
	...
	switch (c)
	{
		...
		case '+':
			if(timeBetweenGenerations>SPEED_INCREMENT){
				timeBetweenGenerations-=SPEED_INCREMENT;
			}
...
// not done yet
void checkPipeCommands(){
	if(pipeFilePointer != NULL){
		char commandString[100];
		if(fscanf(pipeFilePointer, "%s ", commandString) == 1)
		{
			//cyan output!
			printf("\x1b[36m");
			if(strcmp("rule", commandString) == 0){
				char ruleNumber = fgetc(pipeFilePointer);
				
				switch(ruleNumber){
					//	'1' --> apply Rule 1 (Game of Life: B23/S3)
					case '1':
						rule = GAME_OF_LIFE_RULE;
						printf("  changed to life\n");
						break;

					//	'2' --> apply Rule 2 (Coral: B3_S45678)
					case '2':
						rule = CORAL_GROWTH_RULE;
						printf("   changed to coral\n");
						break;

					//	'3' --> apply Rule 3 (Amoeba: B357/S1358)
					case '3':
						rule = AMOEBA_RULE;
						printf("   changed to amoeba\n");
						break;

					//	'4' --> apply Rule 4 (Maze: B3/S12345)
					case '4':
						rule = MAZE_RULE;
						printf("   changed to maze\n");
						break;
					default:
						printf("   %c is not a rule\n", ruleNumber);
						break;
				}
			}else if(strcmp("color", commandString) == 0){
				if(fscanf(pipeFilePointer, "%s", commandString)==1){
					if(strcmp("on", commandString) == 0){
						colorMode = 1;
						printf("   color mode on\n");
					}else if(strcmp("off", commandString) == 0){
						colorMode = 0;
						printf("   color mode off\n");
					}else{
						printf("   bad setting, you said color %s\n",commandString);
					}
				}
			}else if(strcmp("speedup", commandString) == 0){
				if(timeBetweenGenerations>SPEED_INCREMENT){
					timeBetweenGenerations-=SPEED_INCREMENT;
				}
				printf("   speed is %d\n", timeBetweenGenerations);
			}else if(strcmp("slowdown", commandString) == 0){
				timeBetweenGenerations+=SPEED_INCREMENT;
				printf("   speed is %d\n", timeBetweenGenerations);
			}else if(strcmp("end", commandString) == 0){
				// clear the color change
				printf("   C you later!\x1b[0m\n");
				exit(0);
			}else{
						...
						void (*stateDisplayCB)(void))
{
	...
	pipeFilePointer = fopen("/tmp/testpipe", "r");
	int fd = fileno(pipeFilePointer);
