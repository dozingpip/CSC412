...
int main(int argc, const char* argv[]) {
	...
	for(int f = 0; f<numImages; f++){
		processIds[f] = fork();
		if(processIds[f]==0){
			// get the image file path (with the directory and name)
			...
			umask(0);
			mknod(outFileName, S_IFIFO|0666, 0);
			free(outFileName);
	...
	for(int i = 0; i<numImages; i++){
		FILE *fp;
		char* outFileName = (char*) malloc(256*sizeof(char));
		sprintf(outFileName, "p_%d_output", processIds[i]);
		umask(0);
		mknod(outFileName, S_IFIFO|0666, 0);
		...
		fclose(fp);
		free(outFileName);
		waitpid(processIds[i], &statusVal, 0);
	}
